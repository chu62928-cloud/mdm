...
class BabelDataset:
    """Minimal mock to allow running without BABEL labels."""
    @classmethod
    ...
    def from_datasets(cls, datasets):
        return cls()
    def by_babel_sid(self, sid, raise_missing=True):
        return None
    def by_feat_p(self, path, raise_missing=True):
        return None
...
class MSMotionDataset(data.Dataset):
    ...
    def __init__(
        ...
        self.mint_dataset = MintDataset(
            self.mint_root, keep_in_memory=True, pyarrow=False, load_humanml3d_names=True  #
        )
        ...
        #
        logger.info("Building MinT to HumanML3D ID mapping...")
        import pandas as pd
        index_df = pd.read_csv(pjoin(self.motion_root, "index.csv"))
        
        self.mint_to_hml3d = {}
        self.mint_to_offset = {}
        
        for _, row in index_df.iterrows():
            h_id = row['new_name'].replace('.npy', '')
            src = row['source_path']
            self.mint_to_hml3d[src] = h_id
            self.mint_to_offset[h_id] = row['start_frame']

        def map_mint_to_hml3d(mint_id):
            for src, h_id in self.mint_to_hml3d.items():
                if mint_id in src:
                    return h_id
            return None
            
        self.map_mint_to_hml3d = map_mint_to_hml3d
        #
        if DO_PROFILING:
            pr = cProfile.Profile()
            pr.enable()
        ...
        for idx in tqdm(range(len(self.mint_dataset))):
            ...
            # motion_path = pjoin(self.motion_dir, id_name + ".npy")

            # ========== 2. 新增：翻译 ID 并修正过滤逻辑 ==========
            # hml3d_id = map_mint_to_hml3d(id_name)
            hml3d_id = self.map_mint_to_hml3d(id_name)

            if hml3d_id is None:
                # 说明这条 MinT 数据不在 HumanML3D 的 14k 子集中，直接跳过
                index_removal_ids.append(id_name)
                continue

            # 注意：必须用翻译后的 hml3d_id (如 000000) 去和 split 文本比对！
            if self.mode == "train":
                if hml3d_id in non_train_ids:
                    index_removal_ids.append(id_name)
                    continue
            elif self.mode == "val":
                if hml3d_id not in self.val_split_ids:
                    index_removal_ids.append(id_name)
                    continue
            elif self.mode == "test":
                if hml3d_id not in self.test_split_ids:
                    index_removal_ids.append(id_name)
                    continue
            else:
                raise ValueError(f"Invalid mode {self.mode}")

            # 修正读取路径：直接使用 000000.npy
            motion_path = pjoin(self.motion_dir, hml3d_id + ".npy")
            # ====================================================
            ...
            try:
                ...
                logger.warning(f"Problem with {id_name}: {str(e)}")
                index_removal_ids.append(id_name)
    ...
    def __getitem__(self, index, iters=0):
        ...
        # ========== 新增：修正时间戳对齐 ==========
        hml3d_id = self.map_mint_to_hml3d(meta_index)
        start_frame_offset = self.mint_to_offset.get(hml3d_id, 0)
        
        # 将肌肉的绝对时间，减去裁剪偏移量，换算为相对切片索引
        idx = int(window_start_timestamp * self.motion_fps) - int(start_frame_offset)
        
        # 越界保护：如果随机到的肌肉片段超出了动作的有效区间，重新采样
        if idx < 0 or idx + self.window_size > motion.shape[0]:
            new_index = random.randint(0, len(self.data) - 1)
            return self.__getitem__(new_index, iters=iters + 1)
        # ==========================================
