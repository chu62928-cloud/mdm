...
# ★ 改动1：导入 posture guidance 模块
from posture_guidance.mdm_integration import make_fk_fn, apply_posture_guidance
...
def main(args=None):
    ...
    all_motions = []
    all_lengths = []
    all_motions_hml = []      # 新增：原始 hml_vec / 263-d 表示
    all_text = []
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # ★ 改动3：sample_fn 调用新增 posture guidance 参数
        # posture_fk_fn=None 时整个 guidance 分支在 diffusion 内部跳过，
        # 行为与原版完全一致
        sample = sample_fn(
            model,
            motion_shape,
            ...
            noise=None,
            const_noise=False,
            # ---- posture guidance 参数（全部有默认值，不传等同原版）----
        ...
        if model.data_rep == 'hml_vec':
            all_motions_hml.append(sample.detach().cpu().numpy())
    ...
    if len(all_motions_hml) > 0:
        all_motions_hml = np.concatenate(all_motions_hml, axis=0)
        all_motions_hml = all_motions_hml[:total_num_samples]
    else:
        all_motions_hml = None
    ...
    if all_motions_hml is not None:
        save_dict['motion_hml'] = all_motions_hml                                        # [N, 263, 1, T]
        save_dict['motion_hml_tj'] = np.squeeze(all_motions_hml, 2).transpose(0, 2, 1)  # [N, T, 263]

    # posture guidance 元数据（只在启用时写入，方便后续分析复现）
    if posture_instructions is not None:
        save_dict['posture_instructions'] = posture_instructions
        save_dict['guidance_config'] = {
            'lbfgs_steps': getattr(args, 'posture_lbfgs_steps', 5),
...
def is_substr_in_list(substr, list_of_strs):
    return np.char.find(list_of_strs, substr) != -1
