"""Section 2.2 + 3: Freeze configs and launch 12-seed validation.

Methods: V2-fixed, V6-PID, V7.1-A(max=0.10), V7.1-B(max=0.15)
Seeds: 300-311 (12 fresh, non-overlapping)
Targets: 5, 10, 15, 20, 25 deg
"""

import os, sys, json, subprocess, time
from pathlib import Path

PROJ = Path("/root/autodl-tmp/motion-diffusion-model")
OUT = PROJ / "output0727" / "v7_validation"
OUT.mkdir(parents=True, exist_ok=True)

MODEL = os.environ.get("MODEL_PATH", str(PROJ / "save" / "humanml_trans_dec_512_bert" / "model000600000.pt"))
PYTHON = sys.executable

# ---- Frozen config ----
SEEDS = list(range(300, 312))  # 12 fresh
N_SEEDS = len(SEEDS)
SEEDS_STR = ",".join(str(s) for s in SEEDS)

PROMPT = "a person is walking forward"

TARGETS = {
    5:  "anterior_pelvic_tilt_tau05",
    10: "anterior_pelvic_tilt_tau10",
    15: "anterior_pelvic_tilt_tau15",
    20: "anterior_pelvic_tilt",
    25: "anterior_pelvic_tilt_tau25",
}

VARIANTS = [
    ("v2_dps", "V2", {"s": 40, "schedule": "last_quarter"}),
    ("v6_closed_loop", "V6", {"Kp": 80, "Ki": 1, "Kd": 5, "s_min": 0.05, "s_max": 50, "I_max": 20, "beta_ema": 0.8, "lambda_smooth": 0.03, "manifold_project": True, "loss_form": "huber", "huber_delta": 0.05, "normalize_grad": False, "band_gate": False, "spec_schedule_override": "second_half"}),
    ("v7_auto_dps", "V7.1-A", {"schedule": "second_half", "max_backtracks": 3, "trace": False, "max_radius_rms": 0.10}),
    ("v7_auto_dps", "V7.1-B", {"schedule": "second_half", "max_backtracks": 3, "trace": False, "max_radius_rms": 0.15}),
]

n_batches = len(VARIANTS) * len(TARGETS)
print(f"=== V7.1 12-Seed Validation ===")
print(f"Seeds: {SEEDS_STR} (N={N_SEEDS})")
print(f"Targets: {list(TARGETS.keys())}")
print(f"Methods: {[v[1] for v in VARIANTS]}")
print(f"Total batches: {n_batches} (each: {N_SEEDS} seeds)")
print(f"Total runs: {n_batches * N_SEEDS}")
print(f"Output: {OUT}")
print(f"Started: {time.strftime('%H:%M:%S')}")
print()

done, ok = 0, 0
t_total = time.time()

for variant_name, label, kwargs in VARIANTS:
    for target_deg, posture_name in sorted(TARGETS.items()):
        t0 = time.time()
        kwarg_json = json.dumps(kwargs)
        vtag = label.lower().replace(".", "-")
        out_dir = OUT / f"tau{target_deg:02d}" / vtag

        cmd = [
            PYTHON, str(PROJ / "scripts" / "run_seed_batch.py"),
            "--model_path", str(MODEL),
            "--text_prompt", PROMPT,
            "--seeds", SEEDS_STR,
            "--output_dir", str(out_dir),
            "--posture_instructions", posture_name,
            "--variant", variant_name,
            "--variant_kwargs_json", kwarg_json,
            "--guidance_mode", "joint",
            "--motion_length", "6.0",
        ]
        print(f"[{done//N_SEEDS+1}/{n_batches}] {label} tau={target_deg:02d} ({N_SEEDS} seeds)...", flush=True)
        r = subprocess.run(
            cmd, cwd=str(PROJ),
            capture_output=True, text=True, timeout=7200,
        )
        elapsed = time.time() - t0

        n_ok = r.stdout.count("[OK]")
        n_fail = r.stdout.count("[FAIL]")
        n_skip = r.stdout.count("[skip]")
        ok += n_ok
        done += N_SEEDS

        status = "OK" if r.returncode == 0 and n_fail == 0 else "PARTIAL"
        print(f"  [{status}] {n_ok}ok/{n_fail}fail/{n_skip}skip in {elapsed:.0f}s", flush=True)

        # Save log
        log_dir = out_dir / "logs"
        log_dir.mkdir(parents=True, exist_ok=True)
        with open(log_dir / f"log.txt", "w") as f:
            f.write(r.stdout)
            if r.stderr:
                f.write("\n=== STDERR ===\n")
                f.write(r.stderr)

        if r.returncode != 0:
            print(f"  [STDERR] {r.stderr[:500]}", flush=True)

total_elapsed = time.time() - t_total
print(f"\n=== DONE ===")
print(f"Total: {ok}/{done} seeds OK")
print(f"Failed/skipped: {done - ok}")
print(f"Time: {total_elapsed:.0f}s ({total_elapsed/3600:.1f}h)")
print(f"Output: {OUT}")
