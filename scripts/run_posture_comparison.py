...
def parse_extra_args():
    ...
    parser.add_argument("--guidance_mode", type=str, default="joint",
                        choices=["joint", "muscle", "both"])
    parser.add_argument("--muscle_ckpt", type=str, default="")
    parser.add_argument("--muscle_posture", type=str, default="anterior_pelvic_tilt")
    parser.add_argument("--muscle_assets_dir", type=str, default="motion2muscle")
    parser.add_argument("--joint_weight", type=float, default=1.0)
    parser.add_argument("--muscle_weight", type=float, default=1.0)
    parser.add_argument("--proxy_mean_path", type=str, default=None)
    parser.add_argument("--proxy_std_path", type=str, default=None)
...
def main():
    ...
    args.guidance_mode        = extra_args.guidance_mode
    args.muscle_ckpt          = extra_args.muscle_ckpt
    args.muscle_posture       = extra_args.muscle_posture
    args.muscle_assets_dir    = extra_args.muscle_assets_dir
    args.joint_weight         = extra_args.joint_weight
    args.muscle_weight        = extra_args.muscle_weight
    args.proxy_mean_path      = extra_args.proxy_mean_path
    args.proxy_std_path       = extra_args.proxy_std_path
    ...
    # ----- Muscle guidance setup -----
    guidance_mode = getattr(args, "guidance_mode", "joint").lower()
    want_joint = len(args.posture_instructions) > 0 and guidance_mode in ("joint", "both")
    want_muscle = guidance_mode in ("muscle", "both")
    print(f"[Config] mode={guidance_mode} joint={want_joint} muscle={want_muscle}")

    posture_guidance = None
    if want_joint and len(args.posture_instructions) > 0:
    ...
    muscle_guidance = None
    if want_muscle and hasattr(args, "muscle_ckpt") and args.muscle_ckpt:
        ...
        print("[Muscle] guidance instance built")
    elif want_muscle:
        print("[WARN] muscle mode requested but no muscle_ckpt provided, skipping muscle guidance")
    ...
    # ===== Muscle reference: build from baseline output =====
    if want_muscle and muscle_guidance is not None:
        print("\n" + "=" * 70)
        print("Muscle: building reference from baseline sample ...")
        print("=" * 70)
        x0_ref_btc = torch.from_numpy(baseline_out["hml_tj"]).float().to(dist_util.dev())
        muscle_guidance.build_reference(x0_ref_btc)
        print("[Muscle] reference built and frozen.")
    ...
    guided_out = run_single_pass(
        ...
        muscle_weight=args.muscle_weight,
    )
    ...
    print()
    print("Next steps:")
    print(f"  python -m scripts.quantitative_compare {out_path}")
    print(f"  python -m scripts.visualize_compare    {out_path}")
