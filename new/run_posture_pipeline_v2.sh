...
python -m new.quantitative_compare "${NP_PATH}" 2>&1 | tee -a "${LOG_FILE}" | tee "${OUTPUT_DIR}/comparison_report.txt"
log "Step 2 OK"
...
python -m new.visualize_compare "${NP_PATH}" --output_dir "${OUTPUT_DIR}" ${ANIM_FLAG} 2>&1 | tee -a "${LOG_FILE}"
log "Step 3 OK"
