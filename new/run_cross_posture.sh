...
if [ -z "$1" ]; then
    ...
    echo "支持体态: 膝超伸 | 膝弯曲 | 膝弯曲_A | 膝弯曲_B | 骨盆前倾 | 躯干前倾 | 骨盆侧倾 | 驼背 | 头前伸"
    exit 1
fi
...
case "$POSTURE" in
    驼背|头前伸)
        ...
        ;;
    膝超伸|膝弯曲|膝弯曲_A|膝弯曲_B|骨盆前倾|骨盆前倾_深蹲|躯干前倾|骨盆侧倾)
        UNIT_TAG="deg"
        HUBER_DELTA="0.05"
        ;;
    *)
        echo "未知体态 '$POSTURE'。退出。"
        echo "支持: 膝超伸 | 膝弯曲 | 骨盆前倾 | 躯干前倾 | 骨盆侧倾 | 驼背 | 头前伸"
        exit 1
        ;;
esac
...
case "$POSTURE" in
    ...
    躯干前倾)     OUT_TAG="trunk_lean" ;;
    骨盆侧倾)     OUT_TAG="pelvic_tilt_lat" ;;
    驼背)         OUT_TAG="kyphosis" ;;
    头前伸)       OUT_TAG="fhp" ;;
    *)            OUT_TAG="$POSTURE" ;;
esac
